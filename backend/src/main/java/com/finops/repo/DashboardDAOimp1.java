...
public class DashboardDAOimp1 implements DashboardDAO {
    ...
    public int getApprovedLoans() {
        String sql = "SELECT COUNT(*) FROM loan WHERE status='APPROVED'";
        try (Connection con = DatabaseUtil.getConnection();
             PreparedStatement ps = con.prepareStatement(sql);
             ResultSet rs = ps.executeQuery()) {
            if (rs.next()) {
    ...
    public int getRejectedLoans() {
        String sql = "SELECT COUNT(*) FROM loan WHERE status='REJECTED'";
        try (Connection con = DatabaseUtil.getConnection();
             PreparedStatement ps = con.prepareStatement(sql);
             ResultSet rs = ps.executeQuery()) {
            if (rs.next()) {
    ...
    public double getTotalLoanAmount() {
        String sql = "SELECT SUM(amount) FROM loan";
        try (Connection con = DatabaseUtil.getConnection();
             PreparedStatement ps = con.prepareStatement(sql);
             ResultSet rs = ps.executeQuery()) {
            if (rs.next()) {
    ...
    public double getTotalApprovedLoanAmount() {
        String sql = "SELECT SUM(amount) FROM loan WHERE status='APPROVED'";
        try (Connection con = DatabaseUtil.getConnection();
             PreparedStatement ps = con.prepareStatement(sql);
             ResultSet rs = ps.executeQuery()) {
            if (rs.next()) {
