$(document).ready(function () {
    ...
    function loadKycs() {
        ...
        const custMap = {};
        FinOpsStorage.getCustomers().forEach(c => { custMap[c.id] = c; });
    ...
    function uploadKyc() {
        ...
        const docs = [
            { type: 'Aadhaar Card', num: $('#kycAadhaarNum').val().trim(),  file: $('#kycAadhaarFile')[0].files[0] },
            { type: 'PAN Card',     num: $('#kycPanNum').val().trim(),       file: $('#kycPanFile')[0].files[0] },
        ...
        $uploadModal.hide();
        showToast(`${toSave.length} document(s) uploaded. Pending verification.`, 'success');
        refresh();
    }
    ...
    function viewKyc(customerId) {
        const cust    = FinOpsStorage.getCustomer(customerId) || { name: customerId, email: '' };
        const docs    = FinOpsStorage.getKYCDocs().filter(k => k.customerId === customerId);
    ...
    */
    ...
    function populateCustomerDropdown() {
        const opts = FinOpsStorage.getCustomers()
            ...
            .join('');
        $('#kycCustomer').html('<option value="">— Select Customer —</option>' + opts);
    }

    // Bootstrap status badge
    function statusBadge(status) {
        const map = { Verified: 'text-bg-success', Pending: 'text-bg-warning', Rejected: 'text-bg-danger' };
    ...
    function renderPagination() {
        ...
        if (pages <= 1) { $('#kycPagination').html(''); return; }
    ...
    function showToast(message, type = 'info') {
        ...
        if (!$('#toastContainer').length) {
            $('body').append('<div id="toastContainer" style="position:fixed;top:70px;right:20px;z-index:9999;min-width:300px;max-width:400px;"></div>');
        }

        const id  = 'toast-' + Date.now();
        const el  = $(`<div id="${id}" class="alert alert-${type} alert-dismissible d-flex align-items-center gap-2 shadow mb-2" role="alert">
